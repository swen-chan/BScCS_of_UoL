...
function draw() {
	...
	for (var i = 0; i < shopStock.stockLines(); i++) {
		if (shopStock.getStockLevel(i) > 0) {
			...
			var pos_x = shopStock.getLine(i).chocolateBar.pos_x;
			fill(200);
			rect(pos_x, display.y, display.width, display.height);
			fill(0);
			text("Out of Stock", pos_x + display.width / 2,
...
function ChocolateBar(name, imageFile, weight, price, display) {
	...
	this.pos_x = Object.values(display)[0];

	//draw the chocolate bars image to the values set in the display object
	//also display the price and price per 100grams
	this.draw = function() {
		image(this.image, this.pos_x, this.display.y, this.display.width,
			this.display.height);
		var pricePer100g = price / weight * 100;
		var priceString = "£" + price + "p ( £" + round(pricePer100g,2) +
			"p per 100 grams)";
		textAlign(CENTER);
		fill(0);
		text(priceString, this.pos_x + this.display.width / 2,
	...
	this.wasClicked = function(x, y) {
		if (x > this.pos_x && x < this.pos_x + this.display.width &&
