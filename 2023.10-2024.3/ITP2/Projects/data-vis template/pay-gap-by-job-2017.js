function PayGapByJob2017() {
  ...
  this.draw = function() {
    ...
    for (i = 0; i < this.data.getRowCount(); i++) {
      ...
      size = numJobs
      ellipse(
        map(x[i], propFemaleMin, propFemaleMax, this.pad, width),
        map(y[i], payGapMin, payGapMax, height - this.pad, this.pad),
        map(size[i], numJobsMin, numJobsMax, this.dotSizeMin, this.dotSizeMax)
      );
    }
  };
